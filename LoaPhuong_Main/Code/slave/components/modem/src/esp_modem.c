...
#define MODEM_CHECK(a, str, goto_tag, ...)                                              \
    ...
    {                                                                                   \
        if (!(a))                                                                       \
        {                                                                               \
            ESP_LOGI(MODEM_TAG, "%s(%d): " str, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
            goto goto_tag;                                                              \
        }                                                                               \
    } while (0)
...
static esp_err_t esp_dte_handle_line(esp_modem_dte_t *esp_dte)
{
	...
	//Phinht: Debug
//	ESP_LOGI(MODEM_TAG, "dte_handle_line: %s", line);
...
static void esp_handle_uart_pattern(esp_modem_dte_t *esp_dte)
{
	//Phinht: debug
//	const char *line = (const char *)(esp_dte->buffer);
//	ESP_LOGI(MODEM_TAG, "handle_uart_pattern: %s", line);
    ...
    if (pos != -1) {
        ...
        ESP_LOGD(MODEM_TAG, "Pattern Queue Size too small");
...
static void esp_handle_uart_data(esp_modem_dte_t *esp_dte)
{
    ...
    if (length) {
//        pppos_input_tcpip(esp_dte->ppp, esp_dte->buffer, length);	//CRASHED vì cái này khi esp_dte->ppp = NULL vì chưa được khởi tạo!
		if(esp_dte->ppp != NULL) {
//			pppos_input_tcpip(esp_dte->ppp, esp_dte->buffer, length);
			pppos_input(esp_dte->ppp, esp_dte->buffer, length);
		} else
			ESP_LOGE(MODEM_TAG, "esp_dte->ppp is NULL!");
    }
}
...
static void uart_event_task_entry(void *param)
{
    ...
    uart_event_t event;
    while (1) {
        if (xQueueReceive(esp_dte->event_queue, &event, pdMS_TO_TICKS(10))) {	/* 100 */
            switch (event.type) {
            case UART_DATA:
//				ESP_LOGI(MODEM_TAG, "UART_DATA");
                esp_handle_uart_data(esp_dte);	//Crashed vì gọi hàm này!!!
                break;
            case UART_FIFO_OVF:
                ESP_LOGI(MODEM_TAG, "HW FIFO Overflow");
                uart_flush_input(esp_dte->uart_port);
                xQueueReset(esp_dte->event_queue);
                break;
            case UART_BUFFER_FULL:
                ESP_LOGI(MODEM_TAG, "Ring Buffer Full");
                uart_flush_input(esp_dte->uart_port);
                xQueueReset(esp_dte->event_queue);
                break;
            case UART_BREAK:
                ESP_LOGI(MODEM_TAG, "Rx Break");
                break;
...
//				ESP_LOGI(MODEM_TAG, "UART_PATTERN_DET");
                esp_handle_uart_pattern(esp_dte);
                break;
            default:
                ESP_LOGD(MODEM_TAG, "unknown uart event type: %d", event.type);
                break;
            }
        }
        /* Drive the event loop */
        esp_event_loop_run(esp_dte->event_loop_hdl, pdMS_TO_TICKS(5));	/* 50 */
    }
    vTaskDelete(NULL);
}
...
//    ESP_LOGI("MODEM", "UART_WRITE: %d",uart_write_bytes(esp_dte->uart_port, command, strlen(command)));
...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config)
{
    ...
    if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
        res = uart_set_pin(esp_dte->uart_port, MODEM_UART_TX_PIN, MODEM_UART_RX_PIN,
    ...
    if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
		...
		ESP_LOGI(MODEM_TAG, "MODEM_FLOW_CONTROL_NONE");
    ...
    BaseType_t ret = xTaskCreate(uart_event_task_entry,             //Task Entry
                                 ...
                                 8192,	//CONFIG_UART_EVENT_TASK_STACK_SIZE, //Task Stack Size(Bytes) : default 2048
                                 esp_dte,                           //Task Parameter
                                 CONFIG_UART_EVENT_TASK_PRIORITY,   //Task Priority: default 5
...
static void on_ppp_status_changed(ppp_pcb *pcb, int err_code, void *ctx)
{
    ...
    case PPPERR_USER: /* User interrupt */
		ESP_LOGE(MODEM_TAG, "PPP connection user interrupt, try to reopen ppp...");
		/* Free the PPP control block */
//        pppapi_free(esp_dte->ppp);	//Dùng hàm này bị blocking, task phía sau bị block theo!
		ppp_free(esp_dte->ppp);
...
//        pppapi_free(esp_dte->ppp);
    ...
    case PPPERR_CONNECT: /* Connection lost */
		ESP_LOGE(MODEM_TAG, "PPP connection is lost");
		/* phinht: close ppp connection first -> user interrupt will be called later */
//		pppapi_close(esp_dte->ppp, 1);
		ppp_close(esp_dte->ppp, 1);
...
{
    modem_dte_t *dte = (modem_dte_t *)ctx;
	//Phinht: add check dte null
	if(dte == NULL) return len;
...
//    MODEM_CHECK(dte->change_mode(dte, MODEM_PPP_MODE) == ESP_OK, "enter ppp mode failed", err);
	dce->mode = MODEM_COMMAND_MODE;	//phinht: change to command mode before send ATD*99#
	dte->change_mode(dte, MODEM_PPP_MODE);
...
#error "Unsupported AUTH Negotiation"
	...
	ESP_LOGI(MODEM_TAG, "esp_modem_setup_ppp: OK");
    return ESP_OK;
err:
	ESP_LOGE(MODEM_TAG, "esp_modem_setup_ppp: FAILED");
    return ESP_FAIL;
}
...
/*
* Phinht: test close PPP connection
*/
esp_err_t esp_modem_close_ppp(modem_dte_t *dte)
{
    modem_dce_t *dce = dte->dce;
    MODEM_CHECK(dce, "DTE has not yet bind with DCE", err);
    esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
...
//    MODEM_CHECK(pppapi_close(esp_dte->ppp, 1) == ERR_OK, "close ppp connection failed", err);
	MODEM_CHECK(ppp_close(esp_dte->ppp, 1) == ERR_OK, "close ppp connection failed", err);
