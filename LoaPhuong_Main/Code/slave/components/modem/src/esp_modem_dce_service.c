...
#define DCE_CHECK(a, str, goto_tag, ...)                                              \
		...
		{																				  \
			if (!(a))																	  \
			{																			  \
				ESP_LOGW(DCE_TAG, "%s(%d): " str, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
				goto goto_tag;															  \
			}																			  \
		} while (0)
...
esp_err_t esp_modem_dce_sync(modem_dce_t *dce)
{
    ...
    if(dte->send_cmd(dte, "ATV1\r", MODEM_COMMAND_TIMEOUT_DEFAULT) == ESP_FAIL) {
		ESP_LOGD(DCE_TAG,"send command failed");
		return ESP_FAIL;
    }
    if(dce->state != MODEM_STATE_SUCCESS) {
		ESP_LOGE(DCE_TAG, "sync failed");
		return ESP_FAIL;
    }
    ESP_LOGI(DCE_TAG, "Sync OK");
    return ESP_OK;
}
...
esp_err_t esp_modem_dce_echo(modem_dce_t *dce, bool on)
{
    ...
    if (on) {
        ...
        if(dce->state != MODEM_STATE_SUCCESS) {
			ESP_LOGE(DCE_TAG, "enable echo failed");
			return ESP_FAIL;
        }
        ESP_LOGI(DCE_TAG, "enable echo ok");
    } else {
        ...
        if(dce->state != MODEM_STATE_SUCCESS) {
			ESP_LOGE(DCE_TAG, "disable echo failed");
			return ESP_FAIL;
        }
        ESP_LOGI(DCE_TAG, "Disable echo mode OK");
    }
    return ESP_OK;
}
...
esp_err_t esp_modem_dce_store_profile(modem_dce_t *dce)
{
    ...
    ESP_LOGI(DCE_TAG, "Save settings OK");
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
esp_err_t esp_modem_dce_set_flow_ctrl(modem_dce_t *dce, modem_flow_ctrl_t flow_ctrl)
{
    ...
    ESP_LOGI(DCE_TAG, "Set flow control OK");
    return ESP_OK;
err:
    return ESP_FAIL;
}
